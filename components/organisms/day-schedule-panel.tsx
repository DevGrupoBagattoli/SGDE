  ...
  return (
    <section className="rounded-3xl border border-slate-200 bg-white p-4 shadow-sm sm:p-6">
      <div className="flex flex-col gap-2 border-b border-slate-200 pb-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h2 className="text-xl font-semibold text-slate-950">Horários ocupados</h2>
          <p className="text-sm capitalize text-slate-500">
            {longDateFormatter.format(selectedDate)}
          </p>
        </div>
        <span className="inline-flex w-fit items-center gap-2 rounded-full bg-slate-100 px-3 py-1 text-sm font-semibold text-slate-700 ring-1 ring-slate-200">
      ...
      <div className="mt-5 overflow-hidden rounded-2xl border border-slate-200 bg-white">
        {demands.length > 0 ? (
          ...
          <div className="m-3 rounded-xl border border-dashed border-slate-300 bg-slate-50 p-6 text-center text-sm text-slate-500">
